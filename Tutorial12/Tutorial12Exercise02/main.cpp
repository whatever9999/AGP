...
HRESULT InitialiseGraphics()
{
	...
	// Add models to scene
	g_Model0 = new Model(g_pD3DDevice, g_pImmediateContext);
	g_Model0->LoadObjModel((char*)"assets/PointySphere.obj", (char*)"ModelPS", (char*)"ModelVS");
	g_Model0->AddTextures((char*)"assets/BoxTexture.bmp", (char*)"assets/BoxTextureSmiley.bmp");
	g_Model0->SetX(10);
	g_Model0->SetScale(0.5);

	g_Model1 = new Model(g_pD3DDevice, g_pImmediateContext);
	g_Model1->LoadObjModel((char*)"assets/PointySphere.obj", (char*)"ModelPS", (char*)"ModelVS");
	g_Model1->AddTextures((char*)"assets/BoxTexture.bmp", (char*)"assets/BoxTextureSmiley.bmp");
	g_Model1->SetX(-10);
...
void RenderFrame(void)
{
	...
	g_Model0->LookAt_XZ(g_Model1->GetX(), g_Model1->GetZ());
	g_Model0->MoveForward(0.0002);
	if (g_Model0->CheckCollision(g_Model1))
	{
		g_Model0->MoveForward(-0.0002);
	}
	g_Model0->AddAmbientLight(g_ambient_light_colour);
	g_Model0->AddDirectionalLight(g_directional_light_shines_from, g_directional_light_colour, g_rotate_directional_light);
	g_Model0->AddPointLight(g_point_light_position, g_point_light_colour, g_point_light_attenuation);
	g_Model0->Draw(&view, &projection);

	// Pointy sphere following model 0
	g_Model1->LookAt_XZ(g_Model0->GetX(), g_Model0->GetZ());
	g_Model1->MoveForward(0.0001);
	if (g_Model1->CheckCollision(g_Model0))
	{
		g_Model0->MoveForward(-0.0001);
	}
	g_Model1->AddAmbientLight(g_ambient_light_colour);
	g_Model1->AddDirectionalLight(g_directional_light_shines_from, g_directional_light_colour, g_rotate_directional_light);
	g_Model1->AddPointLight(g_point_light_position, g_point_light_colour, g_point_light_attenuation);
