	...
	case WM_LBUTTONUP:
		{
			...
			{
				XMFLOAT4 vertex_color = g_vertices[i].Col;
				for (int j = 0; j < 3; j++)
				{
					vertex_color.x += static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
					vertex_color.y += static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
					vertex_color.z += static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
					if (vertex_color.x > 1.0f) vertex_color.x = 0.0f;
					if (vertex_color.y > 1.0f) vertex_color.y = 0.0f;
					if (vertex_color.z > 1.0f) vertex_color.z = 0.0f;
				}
				g_vertices[i].Col = vertex_color;
			}
