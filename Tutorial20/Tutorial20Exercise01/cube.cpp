...
struct CUBE_CONSTANT_BUFFER // 64 bytes
{
...
HRESULT Cube::LoadCube(char* filename)
{
	HRESULT hr = S_OK;
...
void Cube::RenderCube(XMMATRIX* view, XMMATRIX* projection)
{
	// World View Projection Matrix
	//XMMATRIX world = XMMatrixScaling(0.3f, 0.3f, 0.3f);
	XMMATRIX world = XMMatrixTranslation(1, 0, 15);

	CUBE_CONSTANT_BUFFER cube_cb_values;
	cube_cb_values.WorldViewProjection = world * (*view) * (*projection);

	m_pImmediateContext->VSSetConstantBuffers(0, 1, &m_pConstantBuffer);
	m_pImmediateContext->UpdateSubresource(m_pConstantBuffer, 0, 0, &cube_cb_values, 0, 0);
