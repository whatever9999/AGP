...
// Include for functions to change background colour
...
BackgroundColourFunctions*	g_bcf					= NULL;
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	// Init class for function use
	BackgroundColourFunctions* g_bcf = new BackgroundColourFunctions();
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_LBUTTONDOWN:
		{
			// Get mouse pos to determine alpha of colour change
			POINT mouse_pos;
			mouse_pos.x = LOWORD(lParam);
			mouse_pos.y = HIWORD(lParam);

			// Get the viewports for screen size
			UINT num_viewports = 1;
			D3D11_VIEWPORT viewport;
			g_pImmediateContext->RSGetViewports(&num_viewports, &viewport);

			float screen_size[2];
			screen_size[0] = viewport.Width;
			screen_size[1] = viewport.Height;

			// Change background colour
			g_bcf->MouseClick(g_clear_colour, mouse_pos, screen_size);
			break;
		}
		case WM_PAINT:
